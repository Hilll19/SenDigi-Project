...
const TestUI = () => {
  ...
  const calculateLockedAndScheduledApps = (apps) => {
    ...
    const scheduledApps = apps.filter(
      (app) => app.DateLocked.String || app.TimeStartLocked.String || app.TimeEndLocked.String
    );
  ...
  const calculateOpenedLockApplication = (activities, apps) => {
    ...
    const mostOpenedLockedPackageName = Array.from(occurrences.entries())
      .reduce((max, current) => (current[1] > max[1] ? current : max), [null, 0])[0];
